...
for file in demulti_linked/*_LA.fq; do pre="$(basename $file _LA.fq)"; echo $pre; cutadapt -j ${THREADS} -a file:Barcodes_LA3.fa -a file:Barcodes_LA1.fa -a file:Barcodes_LA2.fa --discard-untrimmed --minimum-length ${MIN_LEN} -o ${pre}_trim1.fq $file; done;
